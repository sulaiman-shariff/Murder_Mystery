...
function Game() {
  ...
  const currentMystery = mysteries.find((mystery) => mystery.id === solvedMysteries.length + 1);

  if (!currentMystery) {
    return <p>🎉 All mysteries solved! Congrats, Detective! 🎉</p>;
  }
  ...
  const sendStatsToBackend = async (mysteryId, time) => {
    const endpoint =
      mysteryId === 3
        ? "http://127.0.0.1:8000/save-stats"
        : "http://127.0.0.1:8000/store_stats";
  ...
  const validateGuess = async () => {
    ...
    if (!currentMystery || !currentMystery.id) {
    ...
    if (isMurdererCorrect && isMotiveCorrect) {
      ...
      setTimeout(() => {
        if (currentMystery.id === 3) {
          navigate("/win");
        } else {
          navigate(`/game${currentMystery.id + 1}`);
        }
      }, 2000);
    } else {
      setFeedback("❌ Wrong answer. Redirecting...");

      await sendStatsToBackend(currentMystery.id, 0); // Only once, after validation fails
    ...
    <div>
      ...
      <div className="game-container">
        ...
        <div className="validation-section">
          ...
          />
          <button onClick={validateGuess}>✔️ Submit Guess</button>
          <p className="feedback-message">{feedback}</p>

          {showNextButton && (
            <button onClick={goToNextGame}>➡️ Proceed to Next Game</button>
          )}
        </div>
