...
const AI_MODEL_ID = process.env.AI_MODEL || "gemini-3.5-flash-lite";
...
export async function callVertexAI(prompt, maxTokens = 1024, temperature = 0.7) {
  if (AI_API_KEY) {
    return callGeminiWithApiKey(prompt, maxTokens);
  }
...
async function callGeminiWithApiKey(prompt, maxTokens) {
  ...
  if (!response.ok) {
    const details = await response.text();
    console.error("Gemini API key request failed:", details);
    throw new Error("Failed to generate AI response");
  }
